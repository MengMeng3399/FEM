...
@ti.kernel
def compute_gradient_co_rotated():
    # clear gradient
...
@ti.kernel
def compute_total_energy_co_rotated():
    for i in range(N_triangles):
        Ds = compute_D(i)
        F = Ds @ elements_Dm_inv[i]
        # co-rotated linear elasticity
...
def compute_total_energy_stVK():
    for i in range(N_triangles):
        ...
        Eye = ti.Matrix.cols([[1.0, 0.0], [0.0, 1.0]])
        stvk = F.transpose() @ F
        element_energy_density = LameMu[None]*0.5*((stvk-Eye)@(stvk-Eye)).trace() + 0.5*LameLa[None]*(stvk-Eye).trace()**2
        total_energy[None] += element_energy_density * elements_V0[i]
    ...
    for i in range(substepping):
        if using_auto_diff:
            ...
            with ti.Tape(total_energy):
                compute_total_energy_co_rotated()
        else:
            compute_gradient_co_rotated()
...
while gui.running:
    ...
    for e in gui.get_events(ti.GUI.PRESS):
        if e.key in [ti.GUI.ESCAPE, ti.GUI.EXIT]:
            ...
            using_auto_diff= not using_auto_diff

        elif e.key == 'a' or e.key=='A':
            use_co_rotated = not use_co_rotated
    ...
    # text
    scheme = 'co_ratated' if use_co_rotated else 'stVK'
    gui.text(
    ...
    gui.text(f'A Scheme={scheme}', pos=(0.6, 0.825), color=0xFFFFFF)
    if damping_toggle[None]:
        gui.text(
    ...
    if using_auto_diff:
        gui.text(
            content='B: using_auto_diff On', pos=(0.6, 0.800), color=0xFFFFFF)
    else:
        gui.text(
            content='B: using_auto_diff Off', pos=(0.6, 0.800), color=0xFFFFFF)
    gui.show()
